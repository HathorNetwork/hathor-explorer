...
function BlueprintList() {
   ...
   *
   * @param {string} type Blueprint type selected
  ...
  const loadData = useCallback(
    async (type, after, before, search, sortOrder) => {
      setLoading(true);
      setError(false);
      try {
        let data;
        if (type === BlueprintType.BUILT_IN) {
          data = await nanoApi.getBuiltInBlueprintList(BLUEPRINT_LIST_COUNT, after, before, search);
        } else {
          data = await nanoApi.getOnChainBlueprintList(BLUEPRINT_LIST_COUNT, after, before, search, null, sortOrder);
        }
        if (before) {
          // When we are querying the previous set of blueprints
          // the API return the oldest first, so we need to revert the list
          reverse(data.blueprints);
        }
        setData(data.blueprints);
   ...
   *
   * @param {string} type The tab that was clicked
   */
  const onTabClicked = (type) => {
    // We reset all parameters
    const paramsToDelete = ['id', 'page', 'sort'];
    const newURL = pagination.setURLParameters({ type }, paramsToDelete);
    history.push(newURL);
  }
  ...
  const renderError = () => {
    return (
      <div className="error-container d-flex">
        <span role="img" aria-label="error" className="mb-3">😞</span>
        <p>Error loading.</p>
        <p>Please try again.</p>
        <button className="retry-button" onClick={handleLoadData}>Try loading again</button>
      </div>
    );
  }
  ...
  const renderNoResult = () => {
    return (
      <div className="no-results-container d-flex">
        <span aria-label="search" role="img" className="mb-3">🔍</span>
    ...
    <div className="section-tables-stylized nano-list-container">
      ...
      <div className="filter-container d-flex flex-row justify-content-between">
        ...
        <div className="tabs-container d-flex flex-row">
          <div className={`tab ${type === BlueprintType.BUILT_IN && 'active'}`} onClick={() => onTabClicked(BlueprintType.BUILT_IN)}>Built In</div>
          <div className={`tab ${type === BlueprintType.ON_CHAIN && 'active'}`} onClick={() => onTabClicked(BlueprintType.ON_CHAIN)}>On Chain</div>
        </div>
        {!isMobile && renderSearch()}
      </div>
      {renderBody()}
